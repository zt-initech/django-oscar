...
class OpenBasketManager(models.Manager):
    ...
    def get_queryset(self):
        return super(OpenBasketManager, self).get_queryset().filter(status=self.status_filter)
...
class SavedBasketManager(models.Manager):
    ...
    def get_queryset(self):
        return super(SavedBasketManager, self).get_queryset().filter(status=self.status_filter)
