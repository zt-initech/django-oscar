...
class AbstractVoucher(models.Model):
    ...
    code = models.CharField(_("Code"),
                            ...
                            db_index=True,
                            unique=True,
                            help_text=_("Case insensitive / No"
                                        " spaces allowed"))
    ...
    def clean(self):
        if all([self.start_datetime, self.end_datetime, self.start_datetime > self.end_datetime]):
    ...
    def is_available_to_user(self, user=None):
        ...
        if self.usage == self.SINGLE_USE:
            ...
            if not user.is_authenticated():
                ...
                is_available = not self.applications.filter(voucher=self, user=user).exists()
                if not is_available:
